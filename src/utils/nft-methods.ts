...
export const handleNftMethods = (methods: any, writeValues: any, constants: Record<string, string> = {}) => {
  ...
  const NFT_METHODS = nftMethods
    .map((method: any) => {
      ...
      const transferMethod = handleTransfer(
        method.decorators,
        struct,
        writeValues,
        variable,
      ...
      const HAS_DISPLAY = method?.decorators
        ?.find((x: any) => x?.name === "Mint")
        .arguments?.find((z: string) => z === "{display: true}");
      if (HAS_DISPLAY) {
        INITS.push(`
